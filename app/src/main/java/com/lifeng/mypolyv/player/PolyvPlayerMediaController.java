...
import com.lifeng.mypolyv.utils.PolyvSensorHelper;
...
public class PolyvPlayerMediaController extends PolyvBaseMediaController implements View.OnClickListener {
    ...
    private void showProgress() {
        if (isShowing && videoView != null) {
            ...
            final int totalTime = videoView.getDuration() / 1000 * 1000;
            ...
            //点击视频后退
            iv_houtui.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (totalTime != 0 && totalTime > 10) {
                        int i = position - 10 * 1000;
                        tv_curtime_land.setText(PolyvTimeUtils.generateTime(i));
                        videoView.seekTo(i);
                        //判断视频状态（是否播放--是后退完继续播放，否后退完暂停）
